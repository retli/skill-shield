...
| 3 | module-biz | 131,000 | 37,700 | 40 | HIGH | Agent 3a~3c | 部分完成(需补扫) | 未开始 |
| 4 | module-common | 3,000 | 600 | 0 | LOW | Agent 4 | 完成 | 未开始 |
| 5 | module-parent | 0 | 0 | 0 | - | - | 跳过 纯POM | - |
```
...
```
### module-biz (131K LOC → 3 个 Agent)
| 子任务 | Agent | 文件范围 | 文件数 | 状态 | 覆盖率 |
|--------|-------|---------|-------|------|-------|
| 3a | Agent 3a | controller/ + service/ | 147 | 完成 | 100% |
| 3b | Agent 3b | dao/ + mapper/ + util/ | 200 | 完成 | 100% |
| 3c | Agent 3c | entity/ + vo/ + query/ | 500 | 部分完成 | 87% |
...
```
收到每个 Agent 结果后立即执行：
    1. 读取 Agent 输出的「文件统计」和「审阅文件清单」
    2. 与实际文件列表交叉验证
    3. 覆盖率 = 100% → 该 Agent 通过
       覆盖率 < 100% → 立即为未覆盖文件启动补扫 Agent（不等其他 Agent 完成）
    4. 更新覆盖矩阵
 
所有 Agent 完成后：
    全部模块 Phase2 = 完成 → 进入 Phase 3
    存在 未开始 或 部分完成 → 启动补充 Agent → 重新过门禁 → 循环直到 100%
...
**第 4 条尤其重要**传统审计报告只有"有漏洞"和"没漏洞"两种状态，但 AI 审计需要第三种状态 — Hypothesis（假设）。当 LLM 发现一个可疑模式但无法完全确认可利用性时（比如调用链中间有一跳追不到，或者不确定某个检测是否有效），标记为 Hypothesis 而不是硬说 Confirmed。这些 Hypothesis 需要人工验证，这样至少不会污染报告的可信度。
...
检查版本：
  ...
  ≥ 1.2.83 → 检查 safeMode 配置：
      ...
      safeMode = false + autoType = false → 需要进一步分析 expectClass 绕过
```
...
```
  ...
  Base Score 不是简单的"RCE = 10 分"，而是按实际影响细分：
...
```
访问路径：互联网(0) / 内网(-2) / 物理(-4)
权限门槛：无需认证(0) / 低权限(-1) / 高权限(-3)
交互复杂度：零交互(0) / 弱交互(-1) / 强交互(-3)
```
...
| 序号 | 字段     | 说明                                                         | 为什么必填                       |
| ---- | -------- | ------------------------------------------------------------ | -------------------------------- |
| 1    | 基本信息 | 漏洞类型+CWE-ID+严重程度+DKTSS评分(含推导公式)+受影响组建+文件:行号 | 让读者快速了解漏洞全貌           |
| 2    | 触发条件 | 漏洞触发的前置条件列表                                       | 明确利用场景,避免"理论漏洞"      |
| 3    | 所需权限 | 利用所需的最低权限(无需认证/低权限/高权限)                   | 评估实战威胁，对应DKTSS Friction |
| 4    | 漏洞原理 | 编号步骤的逐步分析(>=2步)                                    | 让开发理解"为什么有漏洞"         |
| 5    | 代码证据 | 关键代码片段+文件:行号(必须来自实际Read)                     | 反幻觉:可验证的证据              |
| 6    | 调用链   | Source 到 sink 完整链路图，每跳标准文件:行号                 | 证明用户输入确实能到达危险函数   |
| 7    | Poc      | HTTP请求或利用脚本(CONFIRMED 必须可执行)                     | 让开发能复现漏洞                 |
| 8    | 业务影响 | 对业务的实际影响描述                                         | 让管理层理解风险                 |
| 9    | 修复建议 | 具体修复方案(>=1条)                                          | 给出可执行的修复路径             |
...
### VULN-001: Velocity SSTI 导致远程代码执行
#### 基本信息
| 属性           | 值                                                           |
| -------------- | ------------------------------------------------------------ |
| **状态**       | CONFIRMED                                                    |
...
| **CWE-ID**     | CWE-94                                                       |
| **严重程度**   | Critical                                                     |
| **DKTSS评分**  | 9 (Base=10, Friction=-1, Weapon=0, Ver=0)                    |
| **受影响组件** | module-template-engine                                       |
| **文件位置**   | TemplateController.java:45                                   |
#### 触发条件
1. 应用使用 Velocity 模板引擎
2. 用户可控的输入被直接传入 `Velocity.evaluate()`
3. 未配置 SecureUberspector 或配置不当
#### 所需权限
无需认证（前台接口）
#### 漏洞原理
1. `TemplateController.renderTemplate()` 接收用户输入的 `template` 参数
2. 直接将用户输入传入 `Velocity.evaluate(context, writer, "userTemplate", template)`
3. Velocity 默认允许反射调用，攻击者可构造 `#set($x='')#set($rt=$x.class.forName('java.lang.Runtime'))` 执行任意代码
#### 代码证据
...
用户输入 (template 参数)
  ↓
TemplateController.renderTemplate() [TemplateController.java:46]
  ↓
Velocity.evaluate() [velocity-1.7.jar]
  ↓
Runtime.getRuntime().exec() [通过反射调用]
```

 **PoC**

```
POST /api/template/render HTTP/1.1
Host: target.com
Content-Type: application/x-www-form-urlencoded

template=%23set($x%3D%27%27)%23set($rt%3D$x.class.forName(%27java.lang.Runtime%27))%23set($chr%3D$x.class.forName(%27java.lang.Character%27))%23set($str%3D$x.class.forName(%27java.lang.String%27))%23set($ex%3D$rt.getRuntime().exec(%27whoami%27))
```
...
// 安全做法
Template template = ve.getTemplate("predefined-template.vm");
context.put("userContent", sanitizedInput);
template.merge(context, writer);
```

- **纵深防御**：添加 WAF 规则检测 Velocity 注入特征（`#set`、`$rt.getRuntime()`）

  在报告中也会提及之前提及的CONFIRMED和HYPOTHESIS。

```
## 9.5.4 CONFIRMED vs HYPOTHESIS 的区别
 
| 状态 | 定义 | 要求 | 典型场景 |
|------|------|------|---------|
| **CONFIRMED** | 已验证可利用 | PoC 必须可执行，调用链完整，影响明确 | 成功执行 RCE、完成 SQL 注入脱库、绕过认证访问后台 |
| **HYPOTHESIS** | 疑似漏洞，需人工验证 | 发现可疑模式但无法完全确认 | 调用链中间有一跳追不到、不确定某个过滤是否有效、版本信息不明确 |
...
- 语言无关导致深度不足：为了适配多种语言，只能定义通用流程，无法针对 Java 生态的特殊性做深度适配（MyBatis `${}` 注入、Spring Security 路径匹配器差异、Fastjson autoType、Jakarta EE 双命名空间等）
- 缺少漏洞成立条件判断：发现一个 `Velocity.evaluate()` 就报 SSTI，但没有检查 SecureUberspector 是否配置。大量假阳性
- 缺少实战评分体系：要么用 CVSS（不贴合实战），要么用 High/Medium/Low 三级（太粗糙）

**差异化总结**

| 维度          | Prompt模板 | 工具封装         | 通用框架     | 深度垂直Skill          |
| ------------- | ---------- | ---------------- | ------------ | ---------------------- |
| 覆盖率保障    | 无         | 取决于工具       | 有流程无门禁 | 100%门禁强制           |
| 幻觉控制      | 无         | 低(工具输出可信) | 弱           | 5条反幻觉铁律          |
| 资源分配      | 无         | N/A              | 均匀分配     | EALOC公式+Tier 分层    |
| 漏洞判断深度  | 模式匹配   | 工具规则         | 通用规则     | 9类成立条件判断表      |
| 评分体系      | 无/CVSS    | 工具自带         | CVSS/三级    | DKTSS 实战评分         |
| 报告标准化    | 无         | 工具格式         | 简单模板     | 9必填字段+完整性门禁   |
| java 生态适配 | 无         | 部分             | 弱           | 23类漏洞+Jakarta EE    |
| 上下文管理    | 无         | 无               | 弱           | 文件持久化全链路       |
| 大项目适应性  | <10k LOC   | 取决于工具       | 中等         | 多Agent+EALOC 动态分配 |
